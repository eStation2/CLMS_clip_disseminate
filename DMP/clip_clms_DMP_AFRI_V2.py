...
def run_dmp_afri_clipping(filepathname, dir_out="/home/eouser/clms/outputs/afr/"):
    ...
    identifier = 'urn:cgls:africa:dmp300_v2_333m:' + var_name + '300-RT0_' + date_str + '_AFRI_OLCI_' + version
    parent_identifier = 'urn:cgls:africa:dmp300_v2_333m'

    destination_xml = os.path.join(
        dir_out,
        date_str[0:8],
        "c_gls_" + var_name + "300-RT0_PROD-DESC_" + date_str + "_AFRI_OLCI_" + version + ".xml",
    )
    ...
    params_dict = {
        ...
        "alternate_title": var_name + "300-RT0_" + date_str + "_AFRI_OLCI_" + version,
        "product_date": formatted_date,
        "time_coverage_start": time_coverage_start,
        "time_coverage_end": datetime_object.strftime("%Y-%m-%dT23:59:59"),
        "product_version": version,
        # ql_filename is generated internally by clip_and_package
        "ql_filename": "",
    }

    xml_file_path = os.path.join(dir_in, "CGLS_" + var_name + "300_V2_S3_ProductSet_PDF.xml")
    ...
    return clip_and_package(
        ...
        identifier=identifier,
        parent_identifier=parent_identifier,
        color_table=os.path.join(dir_in, "cgl_colorTable_DMP.txt"),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python clip_clms_DMP_AFRI.py <input_directory>")
        sys.exit(1)

    filepathname = sys.argv[1]
    try:
        final_path = run_dmp_afri_clipping(filepathname)
        print(final_path)
        sys.exit(0)
    except Exception as e:
