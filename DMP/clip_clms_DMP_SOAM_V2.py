...
def clip_all_vars_netcdf4(input_nc_file, output_nc_file, lat_range=None, lon_range=None,
    ...
    try:
        with nc.Dataset(input_nc_file, 'r') as src, nc.Dataset(output_nc_file, 'w', format='NETCDF4') as dst:
            ...
            history_entry = f"\n{datetime.today().strftime('%Y-%m-%d')}: CLMS Clipping tool for South-America"
            ...
            for var_name in data_vars:
                ...
                # Format: (Time, Lat, Lon)
                if var_name == "QFLAG":
                    new_chunks = (1, 1920, 1920)
                else:
                    new_chunks = (1, 1344, 1344)
...
def thumbnail_view(filename, thumbFilename, colorTable):
    ...
    qlDict = {
        ...
        "src_Min"       : 0,
        #"src_Max"       : 32767,
        "gdal_open_option" : ["HONOUR_VALID_RANGE=TRUE"] #needed for DMP, if not available, defaults to  ["HONOUR_VALID_RANGE=FALSE"]
    }
...
def run_dmp_soam_clipping(filepathname, dir_out="/home/eouser/clms/outputs/sam/"):
    ...
    # Updated Version and Identifier logic
    version = 'V2.0.1'  # Replaced V1.1.1 with V2.0.1
    v_num = 'v2'        # Replaced v1 with v2
    
    date_str = os.path.basename(filepathname).split("_")[3]
    directory_name = dir_out + date_str[0:8]
    os.makedirs(directory_name, exist_ok=True)
    
    # Updated identifiers with v2 and V2.0.1
    identifier = f'urn:cgls:south-america:dmp300_{v_num}_333m:{var_name}300-RT0_{date_str}_SOAM_OLCI_{version}'
    parent_identifier = f'urn:cgls:south-america:dmp300_{v_num}_333m'
    ...
    clip_all_vars_netcdf4(filepathname, output_file, origin_lat=origin_lat, origin_lon=origin_lon, 
                          clip_width=desired_width, clip_height=desired_height, 
                          data_vars=['DMP', 'LENGTH_AFTER', 'LENGTH_BEFORE', 'NOBS', 'QFLAG', 'RMSE'], 
                          identifier=identifier, parent_identifier=parent_identifier)

    ##### Thumbnail view ####
    parts_thumb = os.path.splitext(output_file)[0].split("_")
    if len(parts_thumb) > 3:
        parts_thumb.insert(3, "QL")
    thumbFilename = "_".join(parts_thumb)
    
    colorTable = os.path.join(dir_in, "cgl_colorTable_DMP.txt")
    ...
    ##### XML ####
    destination_xml = os.path.join(directory_name, f"c_gls_{var_name}300-RT0_PROD-DESC_{date_str}_SOAM_OLCI_{version}.xml")
    ...
    params_dict = {
        ...
        "alternate_title": f"{var_name}300-RT0_{date_str}_SOAM_OLCI_{version}",
    ...
    # Updated XML input path (assuming the template name also changed or needs to match)
    xml_file_path = os.path.join(dir_in, f"CGLS_{var_name}300_V2_S3_ProductSet_PDF_SOAM.xml")
    ...
    return "{}.zip".format(output_zip_name)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python clip_clms_DMP_SOAM.py <input_directory>")
        sys.exit(1)

    filepathname = sys.argv[1]
    try:
        final_path = run_dmp_soam_clipping(filepathname)
        print(final_path)
        sys.exit(0)
    except Exception as e:
