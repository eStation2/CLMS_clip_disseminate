...
def clip_all_vars_netcdf4(input_nc_file, output_nc_file, lat_range=None, lon_range=None,
    ...
    try:
        with nc.Dataset(input_nc_file, 'r') as src, nc.Dataset(output_nc_file, 'w', format='NETCDF4') as dst:
            ...
            new_chunks = (1, 1920, 2160)  # Example: (time_chunk, y_chunk, x_chunk)
...
def run_fapar_afri_clipping(filepathname):
    ...
    colorTable = os.path.join(dir_in, "cgl_colorTable_FAPAR.txt")
    ...
    ##### XML ####
    destination_xml = os.path.join(directory_name, f"c_gls_{var_name}300-RT0_PROD-DESC_{date_str}_AFRI_OLCI_{version}.xml")
