...
def run_fcover_afri_clipping(filepathname, dir_out="/home/eouser/clms/outputs/afr/"):
    ...
    identifier = f'urn:cgls:africa:fcover300_{v_num}_333m:{var_name}300-RT0_{date_str}_AFRI_OLCI_{version}'
    parent_identifier = f'urn:cgls:africa:fcover300_{v_num}_333m'

    destination_xml = os.path.join(
        dir_out,
        date_str[0:8],
        f"c_gls_{var_name}300-RT0_PROD-DESC_{date_str}_AFRI_OLCI_{version}.xml",
    )
    ...
    params_dict = {
        ...
        "alternate_title": f"{var_name}300-RT0_{date_str}_AFRI_OLCI_{version}",
    ...
    xml_file_path = os.path.join(dir_in, f"CGLS_{var_name}300_V2_S3_ProductSet_PDF.xml")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python clip_clms_FCOVER_AFRI.py <input_directory>")
        sys.exit(1)

    filepathname = sys.argv[1]
    try:
        final_path = run_fcover_afri_clipping(filepathname)
        print(final_path)
        sys.exit(0)
    except Exception as e:
