...
def run_lai_afri_clipping(filepathname, dir_out="/home/eouser/clms/outputs/afr/"):
    ...
    identifier = f'urn:cgls:africa:lai300_{v_num}_333m:{var_name}300-RT0_{date_str}_AFRI_OLCI_{version}'
    parent_identifier = f'urn:cgls:africa:lai300_{v_num}_333m'
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python clip_clms_LAI_AFRI.py <input_directory>")
        sys.exit(1)

    filepathname = sys.argv[1]
    try:
        final_path = run_lai_afri_clipping(filepathname)
        print(final_path)
        sys.exit(0)
    except Exception as e:
